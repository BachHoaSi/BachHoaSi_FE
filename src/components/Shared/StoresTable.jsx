...
import { Button, Empty, Form, Input, InputNumber, Modal, Popconfirm, Select, Space, Table, Tag, message } from 'antd';
...
const StoresTable = () => {
    ...
    const handleFilterData = (rank, searchText) => {
        ...
        if (rank !== 'All') {
            filteredData = filteredData.filter(item => item['store-level'] === rank);
        }
        if (searchText) {
    ...
    const handleFormSubmit = async (values) => {
        try {
            ...
            if (selectedStore) {
                await api.put(`/stores?code=${selectedStore.store['store-code']}`, requestBody);
            } else {
